...
def report():
    ...
    lines += ['', '内层梯度消融使用同一外层框架及相同的 2000 步内层上限。若返回 inner_failed，表示在这一预算内未满足残差要求，不能把它记作已收敛的速度比较。记忆规模的单次时间只作敏感性观察。', '',
        ...
        '## 7. 复现', '',
        '在仓库根目录运行 `python reproduce.py supporting`，在输出目录查看报告。运行环境、依赖版本和数据校验值见 `results/manifest.json`。所有主实验的逐步残差、步长、迭代点与证书保存在 `results/*_fixed_*.json`、`results/*_original_*.json`。重复计时原始值见 `results/repeated_timings.json`；源代码为 `run_experiments.py` 和 `analyze_results.py`。', '',
